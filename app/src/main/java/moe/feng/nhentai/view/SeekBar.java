...
import moe.feng.nhentai.drawable.RippleDrawable;
import moe.feng.nhentai.util.ColorUtil;
import moe.feng.nhentai.util.RippleManager;
import moe.feng.nhentai.util.ThemeUtil;
import moe.feng.nhentai.util.TypefaceUtil;
import moe.feng.nhentai.util.ViewUtil;
...
public class SeekBar extends View {
	...
	private void applyStyle(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes){
		...
		mDiscreteMode = a.getBoolean(R.styleable.SeekBar_sl_discreteMode, mDiscreteMode);
		mPrimaryColor = a.getColor(R.styleable.SeekBar_sl_primaryColor, ThemeUtil.colorControlActivated(context, 0xFF000000));
		mSecondaryColor = a.getColor(R.styleable.SeekBar_sl_secondaryColor, ThemeUtil.colorControlNormal(context, 0xFF000000));
		mTrackSize = a.getDimensionPixelSize(R.styleable.SeekBar_sl_trackSize, ThemeUtil.dpToPx(context, 2));
		int cap = a.getInteger(R.styleable.SeekBar_sl_trackCap, 0);
		if(cap == 0)
		...
		mThumbBorderSize = a.getDimensionPixelSize(R.styleable.SeekBar_sl_thumbBorderSize, ThemeUtil.dpToPx(context, 2));
		mThumbRadius = a.getDimensionPixelSize(R.styleable.SeekBar_sl_thumbRadius, ThemeUtil.dpToPx(context, 10));
		mThumbFocusRadius = a.getDimensionPixelSize(R.styleable.SeekBar_sl_thumbFocusRadius, ThemeUtil.dpToPx(context, 14));
		mTravelAnimationDuration = a.getInteger(R.styleable.SeekBar_sl_travelAnimDuration, context.getResources().getInteger(android.R.integer.config_mediumAnimTime));
		mTransformAnimationDuration = a.getInteger(R.styleable.SeekBar_sl_travelAnimDuration, context.getResources().getInteger(android.R.integer.config_shortAnimTime));
		int resId = a.getResourceId(R.styleable.SeekBar_sl_interpolator, 0);
		mInterpolator = resId != 0 ? AnimationUtils.loadInterpolator(context, resId) : new DecelerateInterpolator();
		mGravity = a.getInt(R.styleable.SeekBar_android_gravity, Gravity.CENTER_VERTICAL);
		mMinValue = a.getInteger(R.styleable.SeekBar_sl_minValue, mMinValue);
		mMaxValue = a.getInteger(R.styleable.SeekBar_sl_maxValue, mMaxValue);
		mStepValue = a.getInteger(R.styleable.SeekBar_sl_stepValue, mStepValue);
		setValue(a.getInteger(R.styleable.SeekBar_sl_value, getValue()), false);

		String familyName = a.getString(R.styleable.SeekBar_sl_fontFamily);
		int style = a.getInteger(R.styleable.SeekBar_sl_textStyle, Typeface.NORMAL);

		mTypeface = TypefaceUtil.load(context, familyName, style);
		mTextColor = a.getColor(R.styleable.SeekBar_sl_textColor, 0xFFFFFFFF);
		mTextSize = a.getDimensionPixelSize(R.styleable.SeekBar_sl_textSize, context.getResources().getDimensionPixelOffset(R.dimen.abc_text_size_small_material));
		setEnabled(a.getBoolean(R.styleable.SeekBar_android_enabled, true));
	 ...
	 * @param listener The {@link SeekBar.OnPositionChangeListener} will be called.
	 */
